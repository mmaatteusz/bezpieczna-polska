...
void main() {
  ...
  testWidgets('service incident section and alpha.10 filters are visible', (tester) async {
    ...
    final event={
      ...
      'validTo':null,
      'sources':[
        {'id':'PSP_INCIDENTS','name':'Państwowa Straż Pożarna — zdarzenia','url':'https://www.gov.pl/web/kgpsp/aktualnosci','tier':1}
      ],
      'instructions':<String>[],
    ...
    SharedPreferences.setMockInitialValues({'snapshot::04':jsonEncode(snapshot),'region':'04'});
    final repo=DataRepository(await SharedPreferences.getInstance());
    await tester.pumpWidget(SafetyApp(repository:repo));
    await tester.pumpAndSettle();

    for(var i=0;i<12&&find.text('Zdarzenia służb').evaluate().isEmpty;i++){
    ...
    await tester.fling(find.byType(ListView).first,const Offset(0,2000),1000);
    await tester.pumpAndSettle();

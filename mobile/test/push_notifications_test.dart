...
void main() {
  ...
  test('initialization checks state without requesting system permission', () async {
    final adapter = FakePushAdapter();
    final repository = DataRepository(
      await SharedPreferences.getInstance(),
      buildApi: 'https://api.example',
      client: MockClient((_) async => throw StateError('network not expected')),
    );
  ...
  test('token refresh updates registration without putting token in URL', () async {
  ...
  test('current watched locations are synced as preferences without history', () async {
    ...
    final restarted = PushManager(repository, adapter);
  ...
  test('backend failure is explicit and does not fake a registration', () async {
    final adapter = FakePushAdapter();
    final repository = DataRepository(
      await SharedPreferences.getInstance(),
      buildApi: 'https://api.example',
  ...
  testWidgets('unconfigured preview reports backend and provider honestly', (
    ...
    final manager = PushManager(repository, null);
    await tester.pumpWidget(
      MaterialApp(home: NotificationSettingsScreen(manager: manager)),
    );
    await tester.pumpAndSettle();

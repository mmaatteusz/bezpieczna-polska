...
test('map rejects missing, reversed, malformed bbox, unknown region/filter and excessive zoom',()=>{
 ...
 const neptun=mapLayers.find(l=>l.id==='NEPTUN');assert.equal(neptun?.authority,'OSINT');assert.equal(neptun?.geometry,'COARSE_LIVE_POINTS_AND_HISTORICAL_LINES');assert.equal(neptun?.mode,'LIVE_AND_HISTORY');const ua=mapLayers.find(l=>l.id==='Ukraine_alerts');assert.equal(ua?.enabled,false);assert.equal(mapLayers.filter(l=>l.enabled).length,3);
});

...
test('RCB RSO WCZK: three sources, one incident, stable batch ordering',()=>{const events=[report('RCB'),report('RSO'),report('WCZK-04')];assert.equal(correlate(events).length,1);assert.equal(correlate(events)[0].sourceCount,3);assert.deepEqual(correlate([...events].reverse()),correlate(events));});
...
test('three sources never triple status; unverified RSO is not promoted',()=>{const events=[report('RCB'),report('RSO',{verification:'UNVERIFIED'}),report('WCZK-04')];const s=computeStatus(events,[health],'04',now);assert.equal(s.hazardLevel,'CAUTION');assert.equal(s.incidentCount,1);assert.equal(s.supportingEventIds.length,1);assert.equal(s.supportingIncidents[0].sourceCount,3);assert.equal(s.supportingIncidents[0].confirmedSourceCount,2);assert.equal(events[1].verification,'UNVERIFIED');});
...
test('extension, area correction, ending and retraction retain group and append-only history across restart',async()=>{
 ...
 try{
  ...
  const app=await buildApp(s);try{const snapshot=(await app.inject('/v1/snapshot?regionId=04')).json();assert.equal(snapshot.incidents[0].sourceCount,3);assert.equal(snapshot.incidents[0].reports.length,3);assert.equal(snapshot.events.length,3);const timeline=(await app.inject(`/v1/incidents/${id}/timeline`)).json();assert.equal(timeline.length,7);assert.ok(timeline.some((r:any)=>r.changes.some((ch:any)=>ch.field==='validTo')));}finally{await app.close();}

...
type ShelterMeta={dataDate:string;sourceUpdatedAt:string;count:number|null};

export function parseShelterResource(text:string,now:Date):ShelterMeta&{count:number}{
...
type ZipEntry={name:string;method:number;flags:number;compressedSize:number;uncompressedSize:number;localOffset:number;dosTime:number;dosDate:number};
function zipDate(entry:ZipEntry){
 const day=entry.dosDate&31,month=(entry.dosDate>>5)&15,year=1980+((entry.dosDate>>9)&127);
 const second=(entry.dosTime&31)*2,minute=(entry.dosTime>>5)&63,hour=(entry.dosTime>>11)&31;
 if(year<2020||month<1||month>12||day<1||day>31||hour>23||minute>59||second>59)throw new Error('SHELTER_ARCHIVE_DATE_INVALID');
 const date=new Date(Date.UTC(year,month-1,day,hour,minute,second));
 return {dataDate:date.toISOString().slice(0,10),sourceUpdatedAt:date.toISOString()};
}
export function extractShelterCsv(zip:Uint8Array):{csv:string;dataDate:string;sourceUpdatedAt:string}{
 ...
 for(let i=0;i<count;i++){
  ...
  const flags=b.readUInt16LE(offset+8),method=b.readUInt16LE(offset+10),dosTime=b.readUInt16LE(offset+12),dosDate=b.readUInt16LE(offset+14),compressedSize=b.readUInt32LE(offset+20),uncompressedSize=b.readUInt32LE(offset+24);
  ...
  if((flags&1)!==0||compressedSize===0xffffffff||uncompressedSize===0xffffffff)throw new Error('SHELTER_ARCHIVE_UNSUPPORTED');
  entries.push({name,method,flags,compressedSize,uncompressedSize,localOffset,dosTime,dosDate});offset=end;
 }
 ...
 const entry=candidates[0];if(![0,8].includes(entry.method)||entry.uncompressedSize>64*1024*1024)throw new Error('SHELTER_ARCHIVE_COMPRESSION_UNSUPPORTED');
 ...
 return {...zipDate(entry),csv:new TextDecoder('utf-8',{fatal:true}).decode(raw)};
}

export function parseShelterCsv(csv:string,meta:ShelterMeta,sourceUrl=SHELTER_ORIGIN_CSV):Shelter[]{
 ...
 if(meta.count!==null&&rows.length!==meta.count)throw new Error('SHELTER_COUNT_MISMATCH');
 if(rows.length<1||rows.length>500000)throw new Error('SHELTER_COUNT_INVALID');
 ...
 return rows.map(r=>{
  ...
  return shelterSchema.parse({id,sourceId:'SHELTERS',name:r.Nazwa,sourceType:r['Rodzaj obiektu'],category:'SHELTER_POINT',protectionClass:'UNKNOWN',description:r['Opis ogolny'],address:r.Adres,municipality:r.Gmina,county:r.Powiat,regionId,
   ...
   sourceUrl,datasetUrl:SHELTER_DATASET_PAGE,license:'CC BY 4.0',publisher:'Komenda Główna Państwowej Straży Pożarnej'});
 });
}
...
export const shelterAdapter:SourceAdapter={id:'SHELTERS',version:'psp-current+dane-gov-pl-archive/3.1.0',minSyncIntervalSeconds:21600,
 async sync({now,fetchText,fetchBytes}){
  ...
  return {events:[],shelters,complete:true,coverage:'FACILITY_CATALOG',pagesFetched:4,metadata};
 }
};

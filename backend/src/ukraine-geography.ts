...
const normalize=(s:string)=>s.normalize('NFC').toLowerCase().replace(/[’ʼ`]/g,"'").replace(/\s+/g,' ').trim().replace(/ область$/u,'').replace(/^м\.\s*/u,'');
...
export function enrichUaGeometry(events:Event[],regions:UaRegion[],raw:unknown){
 ...
 return events.map(e=>{
  ...
  // Exact administrative name after removing the explicit oblast/city designator. Never map a district/city to its parent oblast.
